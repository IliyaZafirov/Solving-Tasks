function cardGame(arr) {
    ...
    for (let elem of arr) {
        let [playerName, cardsStr] = elem.split(': ');
        let cards = cardsStr.split(', ');

        if (playerName in playersCards) {
            playersCards[playerName].push(...cards) //спред - разбий масива на отделни елементи и пушни всичко в масива като отделни елементи(а не вкарвай масив в масива)
        } else {
    ...
    for (let [name, deck] of entries) {
        ...
        for (let card of uniqueDeck) {
            ...
            let cardValue = Number(powerValues[power]) * typeValues[type];
            deckValue += cardValue;
        }
